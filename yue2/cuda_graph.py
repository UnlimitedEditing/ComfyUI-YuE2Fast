...
def _flash_builds():
    """ComfyUI-YuE2Fast patch: the op can exist yet raise 'USE_FLASH_ATTENTION was not
    enabled for build'. Probe once so 'auto' falls back to cuDNN/SDPA graphs, not eager."""
    global _FLASH_OK
    if _FLASH_OK is None:
        try:
            q = torch.zeros(1, 2, 64, device="cuda", dtype=torch.bfloat16)
...
class GraphAR:
    ...
    def __init__(self, model, prefixes, max_tokens, *, capture=True, attention_backend="auto", fuse_projections=False):
        ...
        flash = fused and config.head_dim <= 256 and hasattr(torch.ops.aten, "_flash_attention_forward") and (
            "seqused_k" in str(torch.ops.aten._flash_attention_forward.default._schema)) and _flash_builds()
        ...
        if attention_backend == "auto":
            # ComfyUI-YuE2Fast patch: cuDNN SDPA is not universally built either; public SDPA
            # always has a kernel and still runs inside the captured graph.
