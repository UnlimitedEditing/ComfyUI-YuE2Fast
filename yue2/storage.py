...
def model_identity(path, verify=True):
    ...
    entries = {}
    for file in files:
        # ComfyUI-YuE2Fast patch: skip hashing multi-GB weights when verification is off
        # (upstream hashes unconditionally, costing ~10s+ per cold start on ephemeral hosts).
        if not verify:
            entries[file.name] = {"sha256": None, "bytes": file.stat().st_size}
            continue
